...
void
WebGLContext::GenerateMipmap(GLenum texTarget)
{
    if (IsContextLost())
        return;

    if (!ValidateTextureTargetEnum(texTarget, "generateMipmap"))
        return;

    const TexTarget target(texTarget);
    ...
    auto formatUsage = tex->ImageInfoAt(imageTarget, 0).Format();
    if (formatUsage->formatInfo->compression)
        return ErrorInvalidOperation("generateMipmap: Texture data at level zero is compressed.");

    if (IsExtensionEnabled(WebGLExtensionID::WEBGL_depth_texture) &&
        formatUsage->formatInfo->hasDepth)
    {
                                                ...
                                                ErrorResult& rv)
{
    ...
    if (fba.Renderbuffer()) {
        ...
        switch (pname) {
             case LOCAL_GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING_EXT:
                if (IsExtensionEnabled(WebGLExtensionID::EXT_sRGB)) {
                    auto formatUsage = fba.Texture()->ImageInfoBase().Format();
                    if (!formatUsage) {
                    ...
                    auto colorComponentType = formatUsage->formatInfo->colorComponentType;
                    if (colorComponentType == webgl::ComponentType::NormUIntSRGB)
                        return JS::NumberValue(uint32_t(LOCAL_GL_SRGB));
                    else
            ...
            case LOCAL_GL_FRAMEBUFFER_ATTACHMENT_COMPONENT_TYPE: {
                ...
                const auto& imageInfo = fba.Texture()->ImageInfoAt(fba.ImageTarget(),
                                                                   fba.MipLevel());
                auto formatUsage = imageInfo.Format();
                MOZ_ASSERT(formatUsage);

                GLenum ret;
                switch (formatUsage->formatInfo->colorComponentType) {
                ...
                case webgl::ComponentType::NormUIntSRGB:
                    ret = LOCAL_GL_UNSIGNED_NORMALIZED;
                    break;
                ...
                case webgl::ComponentType::SharedExp:
                    MOZ_ASSERT(false, "Unhandled RB component type.");
                    ret = LOCAL_GL_NONE;
                    break;
                }
                         ...
                         ErrorResult& rv)
{
    ...
    if (mBoundReadFramebuffer) {
        webgl::FormatUsageInfo* srcFormat;
        if (!mBoundReadFramebuffer->ValidateForRead("readPixels", &srcFormat))
            return;

        auto type = srcFormat->formatInfo->colorComponentType;
        isSourceTypeFloat = type == webgl::ComponentType::Float;
    } else {
