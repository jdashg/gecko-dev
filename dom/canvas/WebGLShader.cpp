                           ...
                           nsACString* const out_translatedSource)
{
    ...
    if (versionStrStart != std::string::npos) {
        ...
        if (isWebGL2 && SubstringStartsWith(source, versionStrStart, versionStr300es)) {
            glesslVersion = 300;
            versionStrLen = ArrayLength(versionStr300es);

        } else if (SubstringStartsWith(source, versionStrStart, versionStr100)) {
            glesslVersion = 100;
            versionStrLen = ArrayLength(versionStr100);
...
bool
WebGLShader::FindUniformUserNameByMappedName(const nsACString& mappedName,
                                             nsCString* const out_userName) const
{
    ...
    if (!mValidator->FindUniformUserNameByMappedName(mappedNameStr, &userNameStr))
        return false;
