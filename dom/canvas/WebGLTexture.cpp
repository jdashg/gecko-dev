                          ...
                          ErrorResult& out_rv)
{
    TexTarget texTarget;
    WebGLTexture* tex;
    if (!ValidateTexTarget(this, rawTexTarget, "texImage3D", &texTarget, &tex))
        return;

    tex->TexImage3D(texTarget, level, internalFormat, width, height, depth, border,
                    format, type, maybeView, &out_rv);
                           ...
                           GLsizei width, GLsizei height)
{
    ...
    GLenum error = GetAndFlushUnderlyingGLErrors();
    if (error) {
        GenerateWarning("texStorage2D generated error %s", ErrorName(error));
        return;
    }
                           ...
                           GLsizei width, GLsizei height, GLsizei depth)
{
    ...
    if (texTarget != LOCAL_GL_TEXTURE_3D)
        return ErrorInvalidEnum("texStorage3D: target must be TEXTURE_3D.");
    ...
    GLenum error = GetAndFlushUnderlyingGLErrors();
    if (error) {
        GenerateWarning("texStorage3D generated error %s", ErrorName(error));
        return;
    }
