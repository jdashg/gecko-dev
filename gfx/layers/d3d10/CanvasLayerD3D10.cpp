...
CanvasLayerD3D10::Initialize(const Data& aData)
{
  ...
  if (aData.mGLContext) {
    mWebGL = aData.mWebGL;
    NS_ASSERTION(mWebGL, "Must be WebGL.");
    ...
    if (!gfxPrefs::WebGLForceLayersReadback()) {
      if (aData.mGLContext->IsANGLE()) {
        const SurfaceCaps& caps = screen->Factory()->mCaps;
        factory = SurfaceFactory_ANGLEShareHandle::Create(aData.mGLContext,
                                                          caps);
      }
    }

    if (factory) {
      screen->SetFactory(Move(factory));
    }
    ...
    NS_ERROR("CanvasLayer created without mSurface, mDrawTarget or mWebGL?");
  }
